...
const AddExpense = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (type === "Instant" && selectedMembers.length === 0) {
      setError("Please select at least one member for instant expense.");
      return;
    }
    ...
    const expenseData = {
      ...
      category,
      notes,
      selectedMembers: type === "Instant" ? selectedMembers : [],
    };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white shadow-md rounded-xl p-6 mt-20">
        ...
        <div className="flex space-x-2 bg-gray-100 rounded-lg p-1">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium transition ${
              type === "Personal"
                ? "bg-white shadow text-[#003E53]"
                : "text-[#003E53] hover:text-[#003E53]"
            }`}
          >
            Personal
          </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Split Between (only for Instant) */}
        {type === "Instant" && (
          <div>
        ...
        onClick={() => navigate("/myexpense")}
          type="submit"
          className="w-full bg-[#003E53] text-white font-semibold py-3 rounded-md hover:bg-[#006d91] transition"
        >
          Add Expense
        </button>
      </form>
